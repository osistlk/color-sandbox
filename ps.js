...
function generateBarChart(percentages) {
  const chart = Array(10)
    .fill()
    .map(() => Array(20).fill(" ")); // create a 10x20 2D array filled with spaces
...
setInterval(function () {
  ...
  setTimeout(function () {
    ...
    if (cpuPercentages.length > 20) {
      // keep the last 20 measurements
      cpuPercentages.shift();
    }

    const averageCPU =
      cpuPercentages.reduce((a, b) => a + b, 0) / cpuPercentages.length;
