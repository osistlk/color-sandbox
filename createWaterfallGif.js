...
const stream = encoder.createReadStream().pipe(fs.createWriteStream('waterfallEffectGif.gif'));
...
for (let frame = 0; frame < framesPerColor; frame++) {
    for (let y = 0; y < height; y += waterfallHeight) {
        ctx.fillStyle = getColorAtPosition(y + frame * (waterfallHeight / framesPerColor), colors);
